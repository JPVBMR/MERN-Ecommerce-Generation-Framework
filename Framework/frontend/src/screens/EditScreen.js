...
import { STORE_DETAILS_RESET, STORE_UPDATE_RESET } from '../constants/constants'
import { getStoreDetails, updateStoreAction } from '../actions/actions'
...
const EditScreen = () => {
  ...
  /* GET STORE FROM STATE */
  const storeDetails = useSelector((state) => state.storeDetails)
  const { loading, error, store } = storeDetails

  /* GET UPDATE STATUS FROM STATE */
  const storeUpdate = useSelector((state) => state.storeUpdate)
  const {
    loading: loadingStoreUpdate,
    error: errorStoreUpdate,
    success: successStoreUpdate,
  } = storeUpdate

  useEffect(() => {
    if (successStoreUpdate) {
      dispatch({ type: STORE_UPDATE_RESET })
      navigate('/')
    } else {
      if (!store || !store.ownerName || store._id !== storeID) {
        dispatch(getStoreDetails(storeID))
      } else {
        /* STORE DETAILS */
        setTitle(store.title)
        setName(store.ownerName)
        setEmail(store.ownerEmail)

        setSelectedBootstrap(store.selectedBootstrap)
        console.log('BOOT: ', selectedBootstrap)

        setMongoURI(store.mongoURI)

        /* AUTH PAGES */
        setIncludeAuthPages(store.includeAuthPages)
        setShowAppBarInRegister(store.authPages.showAppBarInRegister)
        setShowAppBarInLogin(store.authPages.showAppBarInLogin)

        /* PRODUCT LIST FIELDS */
        setIncludeProductListScreen(store.includeProductListScreen)
        setNavbar(store.productListScreen.navbar)
        setFooter(store.productListScreen.footer)

        setRodape(store.productListScreen.rodape)
        setColor(store.productListScreen.color)
        setTamanhoImagem(store.productListScreen.tamanhoimagem)
        ...
        /* STANDARD USER */
        setLandingPage(store.landingPage)

        /* Home Page Section */
        setIncludeHomeScreen(store.includeHomeScreen)
        setHomeTitle(store.homeScreen.homeTitle)
        setHomeMessage(store.homeScreen.homeMessage)
        setShowAppBarInHome(store.homeScreen.showAppBarInHome)
        setHomeTitle(store.homeScreen.homeTitle)
        setHomeTitle(store.homeScreen.homeTitle)

        /* Cart Screen */
        setIncludeShopCart(store.includeShopCart)
        setShowAppBarInCart(store.cartScreen.showAppBarInCart)
        setShowCartIcon(store.cartScreen.showCartIcon)

        setShowCartIcon(store.cartScreen.showCartIcon)
        setCartMessage(store.cartScreen.cartMessage)

        /* Place Order Screen  */
        setIncludePlaceOrder(store.includePlaceOrder)
        setShowAppBarInPlaceOrder(
          store.placeOrderScreens.showAppBarInPlaceOrder
        )
      }
    }
  }, [store, storeID, dispatch, successStoreUpdate, navigate])
  ...
  const handleCreateStore = (storeId) => {
    dispatch({
      type: STORE_DETAILS_RESET,
    })
    navigate(`/`)
  ...
  return (
    <>
      <Row className='mainRow'>
        <Col md={3} ld={1}>
          <StoreListScreen
            onClickCreateStore={handleCreateStore}
        ...
        <Col>
          <>
            ...
            <div className='mx-5'>
              {loadingStoreUpdate && <CustomSpinner />}
              {errorStoreUpdate && (
                <CustomMessage variant='danger'>
                  {errorStoreUpdate}
                </CustomMessage>
              )}
              {loading ? (
                  ...
                  <>
                    ...
                    >
                      ...
                      >
                        <path
                          fill='none'
                          d='M9 14H15V19H9zM11 5H13V7H11z'
                        ></path>
                        <path
                          fill='none'
