...
const BootstrapSelectorCard = (props) => {
  const { selectedBootstrap, setSelectedBootstrap } = props

  const availableBootstrapNames = ['darkly', 'sketchy', 'flatly']

  const handleCardClick = (theme) => {
    setSelectedBootstrap(theme)
  }
        ...
        <Card.Body>
          <Card.Title>
            <h2>
              ...
              <span className='ml-3'>Bootstrap Theme</span>
            </h2>
          </Card.Title>
          <div className='row'>
            {availableBootstrapNames.map((theme) => (
              <div className='col px-3' key={theme}>
                <Card
                  className={`clickable-card ${
                    selectedBootstrap === theme ? 'selected' : ''
                  }`}
                  onClick={() => handleCardClick(theme)}
                >
                  <img
                    src={`http://localhost:3000/${theme}.png`}
                    alt={`${theme} Theme`}
                    className='img-fluid'
                  />
                  {selectedBootstrap === theme && (
...
export default BootstrapSelectorCard
